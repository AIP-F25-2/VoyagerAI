...
def home(city): 
    return f"{BASE}/explore/home?city={slug(city)}"

def events(city): 
    return f"{BASE}/explore/events-{slug(city)}"
...
def scrape_bookmyshow_events(city="Mumbai", limit=10):
    ...
    with sync_playwright() as p:
        ...
        if not ok: 
            print(f"[warn] home nav failed: {err}")
        ...
        time.sleep(0.3 + random.uniform(0.1, 0.4))
        if not ok: 
            print(f"[warn] events nav failed: {err}")
        ...
        for i, url in enumerate(links[:limit], 1):
            ...
            consent(page)
            time.sleep(0.35 + random.uniform(0.05, 0.35))
            if not ok:
